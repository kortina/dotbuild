    ...
    def test_read(self):
        ...
        with patch('os.walk', new=mocks.mock_walk):
            with patch('__builtin__.open', new=mocks.mock_open, create=True):
                r.read()
        self.assertEqual(r.dotfiles['inputrc'].dotfiles[0].dirpath,
                         "./dotfiles-danny")
        self.assertEqual(r.dotfiles['inputrc'].dotfiles[0].contents,
                         "Space: magic-space")
        self.assertEqual(r.dotfiles['bashrc'].dotfiles[0].dirpath,
                         "./dotfiles-z-team")
        self.assertEqual(r.dotfiles['inputrc'].user_dotfile.dirpath,
                         "./dotfiles-user")
